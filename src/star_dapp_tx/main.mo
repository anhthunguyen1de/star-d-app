import AccountId "../star_dapp_backend/utils/accountid";
import Ledger "../star_dapp_backend/ledger";
import Queue "../star_dapp_backend/utils/Queue";
...
import Types "../star_dapp_backend/types";
import Time "mo:base/Time";
import Prim "mo:prim";

shared ({ caller = owner }) actor class StarDAppTxActor() {
  ...
  private stable var star_dapp_canister_id_ : Principal = owner;
  ...
  public shared ({ caller }) func setStarDAppCanisterId(token : Principal) : async Bool {
    assert (caller == owner_);
    star_dapp_canister_id_ := token;
    return true;
  };

  public shared ({ caller }) func newTxRecord(who : Principal, to : Principal, vid : Nat, price : Nat64) : async IIPayInfo {
    assert (caller == star_dapp_canister_id_);
    let now = Time.now();
  ...
  public shared query ({ caller }) func getTxByUser(who : Principal) : async [TxRecord] {
    assert (caller == star_dapp_canister_id_ or caller == owner_);
    var txall : [TxRecord] = [];
    switch (user_txs_map.get(who)) {
      case (?txs) {
        for (v in txs.vals()) {
