...
import StarDAppTx "../star_dapp_tx/main";

shared ({ caller = owner }) actor class StarDApp() {
    ...
    private stable var txActor : ?StarDAppTx.StarDAppTxActor = null;
    ...
    private stable var StarDAppTx_Canister_Id : Text = "";
    ...
    private func getTxActor() : async StarDAppTx.StarDAppTxActor {
        switch (txActor) {
            case (?la) la;
            case _ {
                let la : StarDAppTx.StarDAppTxActor = actor (StarDAppTx_Canister_Id);
                txActor := ?la;
                la;
            };
        };
    ...
    public shared ({ caller }) func setStarDAppTxCanisterId(token : Text) : async Bool {
        assert (caller == owner_);
        StarDAppTx_Canister_Id := token;
        return true;
    };
};
