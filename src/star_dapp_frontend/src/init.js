...
export const star_dappjs = {
  ...
  setuser(user) {
    ...
    if (!this.isauth()) {
      ...
      let str = this.user;
      $("#order").html(
        str.substr(0, 11) + "..." + str.substr(-8) + '<i class="down"></i>'
      );
      $("#wallectConnect").hide();
      $("#order").show();
    }
  ...
  lockShowTimeII() {
    ...
    for (var i = 0; i < this.iilist.length; i++) {
      if (this.iilist[i].locked) {
        ...
        if (
          this.iilist[i].lockSecond <= 0 &&
          $(`#ii-item-${id} .locked`).length > 0
        ) {
          setTimeout(() => {
            $(`#ii-item-${id}`).append(
              `<div class="buy" data="${id}"> BUY </div>`
            );
  ...
  renderII(total, lists) {
    ...
    $(".searchfld input").attr(
      "placeholder",
      `${total_str} results, Please Input II Number`
    );
  ...
  lockII(id) {
    ...
    $(`#ii-item-${id}`).append(
      `<div class="locked"></div><div class="lockbtn"><span class="showtimes">150</span>S Unlock</div>`
    );
    $(".cancel").click();
  },
  ...
  renderTxErr() {
    ...
    $("#ii-order-list").html(
      "<tr><td colspan='6'>load error, please click the refresh again! </td></tr>"
    );
  },
